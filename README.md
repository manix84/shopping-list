...
Unknown product reporting is optional. When enabled, items that parse into the `Other` section are reported from the frontend to the backend with only the item text, normalized item text, country profile, and app language. The backend appends those reports to a GitHub issue titled `Unknown products`.

```text
UNKNOWN_PRODUCTS_GITHUB_REPO=owner/repo
UNKNOWN_PRODUCTS_GITHUB_TOKEN=github_pat_...
```

`UNKNOWN_PRODUCTS_GITHUB_REPO` falls back to `GITHUB_REPOSITORY` when available. The token needs permission to read and write issues for that repository.

Optional overrides:

```text
UNKNOWN_PRODUCTS_GITHUB_ISSUE_NUMBER=123
UNKNOWN_PRODUCTS_GITHUB_ISSUE_TITLE=Unknown products
```
