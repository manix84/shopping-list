...
Use these instructions for all reviews. Focus on bugs, regressions, missing tests, accessibility, persistence, and production safety. Avoid broad style feedback unless it affects maintainability or user behaviour.
...
- Preserve local-first behaviour. Browser storage must work without the backend.
...
- Local Docker Postgres credentials are development-only examples.

## UI And Accessibility

- Prefer existing UI patterns for buttons, cards, fields, selects, status indicators, dialogs, and title rows.
...
For i18n changes, check keys across locales and localized country labels from `messages.countryOptions`.

## Country Profiles

Country profiles control store order, section keywords, measurement behaviour, and defaults. Changes under `src/config/countries` need focused tests. Sort localized country dropdown options using the active app locale.
